...
import (
	...
	"github.com/mojocn/recommend-go/rawData"
	"log"
)
...
func csvDemo() {
	...
	rowSlice ,colSlice, Q := rawData.ReadScoreCsvToMatrix()

	fmt.Println(len(rowSlice))
	fmt.Println(len(colSlice))
	...
	Qhat ,ff := Train(Q, n_factors, n_iterations, lambda)
	fmt.Println(Qhat ,ff)

	// Get Prediction for a user/product pair.
	if ff,err := Predict(Qhat, 2, 1); err == nil {
		fmt.Println(ff)
	}else {
		log.Fatal(err)
	}
	...
	//fmt.Println(GetTopNRecommendations(Q, Qhat, 1, 2, colSlice))
